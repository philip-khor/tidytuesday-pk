...
week2shape$pay.av <- ave(week2shape$pay, week2shape$time, week2shape$year, 
                         FUN = function(x) mean(head(sort(x, decreasing = TRUE), 16), na.rm = TRUE))

week2shape$offense <- grepl('RB|QB|LN|TE|WR', week2shape$time)*1

week2shape$players <- factor(x = week2shape$time, 
...
ggplot(data = week2shape) + 
  ...
  scale_alpha(range = c(0.2, 0.1)) + facet_wrap(~players,nrow = 2) + 
  theme_minimal() + 
...
ggsave('tidy2.png', height = 5, width = 7)
```